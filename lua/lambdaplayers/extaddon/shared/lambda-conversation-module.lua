...
local function FidgetAnimation( self )
	if wOS then
		-- The Simpsons Hit and Run Dialogue Animations
		-- https://steamcommunity.com/sharedfiles/filedetails/?id=3213611699
		local AnimTable_SHR = {
			"dialogue_cross_arms",
		...
		-- TODO: Don't play animations over our current one

		-- Randomly play some SHR animations when talking / listening
		self:PlayGestureAndWait( AnimTable_SHR[ LambdaRNG( #AnimTable_SHR ) ], LambdaRNG( 0.75, 1.25 ) )

		--[[local seq = self:LookupSequence( AnimTable_SHR[ LambdaRNG( #AnimTable_SHR ) ] )
		if seq > -1 then
			self:AddGestureSequence( seq )
			self:SetPlaybackRate( LambdaRNG( 0.75, 1.25 ) )
		end ]]
	end
end
...
local function Initialize( self, wepent )
	...
	function self:Conversation()
		...
		-- Don't randomly crouch
		self.Crouch = false

		-- Don't play any idle sounds
		self.l_nextidlesound = math.huge 

		local partner = self.lc_group[ LambdaRNG( #self.lc_group ) ]
			...
			self:Hook( "PlayerSay", "endtext", function( ply )
				...
				if ply.lc_canspeak then
					...
					if LambdaRNG( 1, 2 ) == 1 then
						partner = self.lc_group[ LambdaRNG( #self.lc_group ) ] 
		...
		if self:GetRangeSquaredTo( partner ) > ( 125 * 125 ) then
			self.lc_movementtbl.callback = function() 
				if IsValid( partner ) and self:GetRangeSquaredTo( partner ) <= ( 125 * 125 ) then
					self:CancelMovement()
				end
			end
		...
		if !self.lc_respondent or LambdaRNG( 100 ) <= 1 then
		...
		local AnimRNG = LambdaRNG( 1, 2 )
		if LambdaRNG( 1, 3 ) == 1 then
			if AnimRNG == 1 then 
				FidgetAnimation( self ) -- The talker plays an animation
			else
				FidgetAnimation( partner ) -- The listener plays an animation
			end
		end
	...
	function self:StartConversation( ent )
		...
		if #self.lc_group == 0 and ( !ent.lc_group or #ent.lc_group == 0 ) then
			...
			self.lc_canspeak = true
			self.lc_maxspeaktimes = LambdaRNG( 3, 20 )
			self.lc_currentspeaktimes = 0

			ent.lc_canspeak = false
			ent.lc_maxspeaktimes = LambdaRNG( 3, 20 )
...
local function LookConversation( self )
	if LambdaRNG( 1, 2 ) != 1 then return end
